...
const CollectionFlow: FC<PageProps> = ({ title }) => {
  ...
  const formItemOptions = (isEdit: string) => [
    {
      label: formatMessage(`workflow.${isEdit}Workflow`),
    },
    {
      ...
      >
        <ComCardList
          ...
          runStatusOptions={[
            ...
            {
              value: 'STOPPED',
              text: formatMessage('workflowEditor.suspend'),
              bgType: 'red',
            },
            {
              value: 'DRAFT',
          ...
          viewOptions={[
            {
              label: formatMessage('workflow.runningInstance'),
              valueKey: 'num',
              render: (text) => {
                return text > 0 ? (
                  <Button style={{ height: 'auto', padding: 0 }} type="link">
                    {text}
                  </Button>
                ) : (
                  0
          ...
          operationOptions={[
            ...
            },
            {
              ...
              onClick: (item: any) => {
                navigate(
                  `/workflow-editor?${getSearchParamsString({ id: item.id, name: item.name, status: item.status, flowId: item.flowId })}`
                );
              },
              type: 'blue',
